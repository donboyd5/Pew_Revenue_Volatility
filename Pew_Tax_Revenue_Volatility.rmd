...
+   creates some derived data files
...
[Reserve fund example with real data for the two different volatility measures.]

## Formal measures of volatility

In his section we compute summary volatility measures by state and tax. We summarize them over the last approximately 30 years (1990-2018) as follows:
...
For this we calculate rolling volatility measures for each state and tax, looking back over 15 years. Thus in each year (for each state-tax combination), we look back 15 years to compute the standard deviation of the percent change, the standard deviation of the percent deviation from trend, and the short-run elasticity.
...
p <- vol_roll %>%
  ...
  left_join(stcodes2 %>% select(stabbr, beargn.name, cenrgn.name)) %>%
