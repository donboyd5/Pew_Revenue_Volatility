...
p <- df %>%
  ...
  # annotate("text", x=1955, y=median(anno$cggdp, na.rm=TRUE) +.005, label = "median", size=2.5) +
  annotate("text", x=1955, y=median(.data$cggdp / 100, na.rm=TRUE) +.005, label = "median", size=2.5) +
  annotate("text", x=1955, y=median(.data$cggdp, na.rm=TRUE) +.005, label = "median", size=2.5) +
  # annotate("text", aes(x = year[year==1985], y = cggdp[year==1985]), label = "Some text")
...
grate <- .03
set.seed(1)
grvec <- (1 + grate)^(0:(nyears - 1))
noise <- rnorm(nyears, mean=0, sd=1)
shock_pattern <- c(0, 0, 0, 0, 0, .1)

pattern_smooth <- seq(.10, -.10, length.out=10)
# shock_smooth <- rep_len(pattern_smooth, nyears)
shock_smooth <- rep_len(c(pattern_smooth, -pattern_smooth), nyears)

shock_smooth

df <- tibble(time=1:nyears + 2024,
             trend=base*grvec) %>%
  mutate(tnoise=trend * (1 + noise/25),
         tsmooth=trend * (1 - shock_smooth)* (1 + noise/50))
...
dfl <- df %>%
  ...
  group_by(time) %>%
  mutate(diff=value - value[name=="trend"],
         pdiff=diff / value[name=="trend"] * 100) %>%
  ungroup
