...
savefig(basename="fig_trend_cycle_random", .p=p7, .pdata=pdata,
        width=8, height=8)
...
savefig(basename="fig_caiit_3panel", .p=p, .pdata=caiit,
        width=8, height=8)
...
Often when inflation spiked, real vol was HIGHER than nom vol.

```{r fig_gdppi_taxpdtrend}

# compare inflation and tax volatility nom vs real
pdata <- vbase %>%
  filter(stabbr=="US", name %in% c("tottax", "gdppi")) %>%
  mutate(value=ifelse(realnom=="price", pch, pdtrend),
         name=factor(realnom,
                     levels=c("price", "nominal", "real"),
                     labels=c("Inflation", "Volatility (nominal)", "Volatility (real)"))) %>%
  select(year, name, realnom, value) %>%
  arrange(name, year)


subt1 <- "Inflation is year over year percentage change in GDP price index."
subt2 <- "Volatility is percentage deviation from trend."
subt3 <- "Recession periods are shaded."
subt <- paste0(subt1, "  ", subt2, "  ", subt3)
...
p <- pdata %>%
  ...
  scale_y_continuous(name="percentage",
                     ...
                     labels=label_percent(accuracy=1)) +
  scale_x_continuous(name=NULL, breaks=seq(1960, 2030, 5)) +
  scale_colour_manual(values=c(bluegreen, darkred)) +
  geom_hline(yintercept = 0) +
  ggtitle("Inflation and tax revenue volatility (nominal and real) in the United States",
          subtitle=subt) +
  labs(caption="Source: U.S. Bureau of the Census and Bureau of Economic Analysis") +
  theme_bw() +
  theme(plot.caption = element_text(hjust=0, size=9)) +
...
savefig(basename="fig_gdppi_taxpdtrend")

```


```{r fig_gdppi_tax_us, include=TRUE, eval=TRUE}

count(vbase, name)
vbase %>% filter(name=="gdppi")

# compare inflation and excess tax volatility nom vs real
pdata <- vbase %>%
  filter(stabbr=="US", name %in% c("tottax", "gdppi")) %>%
  mutate(value=ifelse(realnom=="price", pch, pdtrend)) %>%
  select(year, realnom, value) %>%
  pivot_wider(names_from = realnom) %>%
  mutate(diff=nominal - real) %>%
  pivot_longer(-year)

pdata %>%
  group_by(name) %>%
  summarise(mdn=median(value, na.rm = TRUE),
            iqr=IQR(value, na.rm = TRUE))

pdata %>%
  mutate(inflation=ifelse(year <= 1987, "high", "low")) %>%
  group_by(inflation, name) %>%
  summarise(mdn=median(value, na.rm = TRUE),
            iqr=IQR(value, na.rm = TRUE)) %>%
  pivot_longer(c(mdn, iqr), names_to = "measure") %>%
  pivot_wider(names_from = inflation) %>%
  select(measure, name, low, high) %>%
  mutate(change=high - low) %>%
  arrange(measure, name)

p <- pdata%>%
  # select(year, price, diff)  %>%
  # filter(name %in% c("price", "nominal", "diff")) %>%
  filter(name %in% c("price", "diff")) %>%
  ggplot(aes(year, value, colour=name)) +
  geom_line() +
  geom_point() +
  geom_hline(yintercept = 0) +
  geom_hline(yintercept = pdata %>% 
               filter(name=="nominal") %>% 
               summarise(iqr=IQR(value, na.rm=TRUE)) %>%
               pull(iqr))  
p

# compare trend inflation and pdtrend tax
pdata <- vbase %>%
  filter(stabbr=="US", name %in% c("tottax", "gdppi")) %>%
  mutate(value=ifelse(realnom=="price", pchtrend, pdtrend))

pdata %>%
  ggplot(aes(year, value, colour=realnom)) +
  geom_line() +
  geom_point() +
  geom_hline(yintercept = 0)
...
pdata %>%
  mutate(pdtrend=ifelse(realnom=="price", -pdtrend, pdtrend)) %>%
  ggplot(aes(year, pdtrend, colour=realnom)) +
  geom_line() +
  geom_point() +
  geom_hline(yintercept = 0)
...
p <- df %>%
  ...
  legend_notitle
p 
ggsave(filename = here::here("results", "fig_gdppi_tax_us.png"), 
       plot = p, 
       width=8, height=6)
...
# savetab(basename="tab_pdtrend_taxtype_box")
...
shares <- taxshares %>%
  ...
  select(-year) %>% 
  pivot_longer(cols=-stabbr) %>%
  mutate(share=ifelse(name=="tottax", 1, value))
shares  
shares %>% filter(name!="tottax") %>% summarise(sum(share))
...
tabdata <- volall %>%
  filter(period=="2000-2020", 
         ...
         stabbr=="US", 
         name %in% taxnames$name)  %>%
  left_join(shares %>% select(stabbr, name, share), 
            by = c("stabbr", "name")) %>%
  select(stabbr, period, realnom, name, namef, share, pdtrendiqr) %>%
  arrange(namef)
  
tabdata

tab <- tabdata %>%
  select(namef, share, pdtrendiqr) %>%
  gt() %>%
  tab_header(
    title = "Volatility by tax type for the U.S. as a whole",
    subtitle = "Percent deviation from trend, 2000-2020"
    ) %>%
  cols_label(namef="Type of tax",
             share=html("% share of<br>total tax<br>revenue (2020)"),
             pdtrendiqr=html("% deviation<br>from trend (IQR)")) %>%
  cols_align(
    align = c("left"),
    columns = namef) %>%
  fmt_percent(
    columns = c(share, share, pdtrendiqr),
    decimals = 1) %>%
  tab_style(
    style = list(
...
savetab("tab_pdtrend_all_taxes")

```


```{r tab_all_taxes_multmeasures}
...
savetab("tab_all_taxes_multmeasures")
# 
...
pdata <- vbase %>%
  filter(stabbr=="US", name %in% c("pi", "agi")) %>%
...
```{r tab_agi2010}
# show 2010 data mentioned in the text, in a table
# CAUTION: be sure pdata is the pdata defined immediately above
tabdata <- pdata %>%
  filter(year==2009) %>%
...
tab <- tabdata %>%
  ...
  cols_hide(name) %>%
  tab_header(
    title = "Adjusted gross income in 2010",
    subtitle = NULL
    ) %>%
...
```{r eval=FALSE}
# come back to this?
# capital gains and tax revenue
count(vbase, name)
sts <- c("US", "CA", "NY", "MA")

moddata <- vbase %>%
  filter(realnom=="nominal", name %in% c("iit", "gdp", "capgains")) %>%
  select(stabbr, name, year, pch) %>%
  pivot_wider(values_from = pch) %>%
  group_by(year) %>%
  mutate(capgains=capgains[stabbr=="US"],
         lcapgains=lag(capgains),
         lcapgains=ifelse(stabbr=="NY", lag(lcapgains), lcapgains))

# run a model of iit pch as a function of pch in gdp and lcapgains
# exclude states without a significant income tax
nonpitstates <- c("AK", "FL", "NH", "NV", "SD", "TN", "TX", "WA", "WY")
mods <- moddata %>%
  filter(!stabbr %in% nonpitstates) %>%
  na.omit() %>%
  arrange(stabbr, year) %>%
  nest(data=-c(stabbr)) %>% # nest everthing by state
  mutate(iitmod = purrr::map(
    data, safely(function(df) lm(iit ~ gdp + lcapgains, data=df))))

# get main stats for each state
mstats <- mods %>%
  mutate(
    # result will have the full model results if it was successful
    result=purrr::map(iitmod, "result"),
    # tidied will have multiple rows (collapsed)
    # each row has coefficient, std error, tstat, and pvalue of a var in model
    tidied=purrr::map(result, tidy),
    # stats will have 1 row per state, with model summary stats (e.g., r-squared)
    stats=purrr::map(result, glance))

# find the states in which capital gains was most important, and least important
cgstats <- mstats %>%
  hoist(stats, nobs="nobs", r2adj="adj.r.squared") %>%
  unnest(tidied) %>%
  filter(term=="lcapgains") %>%
  select(stabbr, nobs, r2adj, estimate, std.error, statistic, p.value)

cgstats %>%
  arrange(desc(estimate))

sts <- c("CT", "PA", "RI", "CA", "ND", "NM", "NY", "OH", "MS")
...
pdata %>%
  filter(name != "capgains") %>%
  filter(year >= 1980) %>%
  ggplot(aes(year, value, colour=name)) +
  geom_line() +
  geom_point() +
  scale_y_continuous(limits=c(-.5, .5)) +
  facet_wrap(~stabbr, scales = "free")
...
```{r fig_iitmod_cgresids_2panel}
# for this figure, we need to regress income tax on gdp (for the U.S.),
# because we want the residuals -- the portion of income tax variation not explained 
# by gdp variation

# we will compare the unexplained variation in income tax to the variation in capital gains

# we'll have two panels
#   top panel is before 1986
#   bottom pane is 1986-plus

yearplus <- 1986

moddata1 <- vbase %>%
    filter(name=="iit", realnom=="nominal", stabbr=="US") %>%
...
vbase %>%
  filter(realnom=="nominal", name=="iitpew", stabbr=="CA")
pewtax
count(pewtax, taxtype)
count(pewtax, vartype)
pewtax %>% filter(stabbr=="CA", taxtype=="iit")

gst1 <- vbase %>%
  filter(realnom=="nominal",
         name %in% c("gst", "gstpew", "gstadj", "gstbase")) # we only have US for gst

count(gst1, name)

# let's make a data frame where , for each state, we have all 3 for all years of the data
gst2 <- gst1 %>%
...
tot1 <- vbase %>%
  filter(realnom=="nominal",
         name %in% c("tottax", "tottaxpew")) # we only have US for gst


st <- "TX"
tot1 %>%
  filter(stabbr==st) %>%
  filter(year %in% 1994:2014) %>%
  ggplot(aes(year, pdtrend, colour=name)) +
  geom_line() +
  geom_point() +
  geom_hline(yintercept = 0) +
...
```{r recession_nonrecession}

rrange <- 3  # find all years within +/- rrange of a recession
recyears <- recessions %>% 
  select(year=rec_year) %>%
  filter(year >= 1960) %>%
  group_by(year) %>%
  summarise(year=(year - rrange):(year + rrange), .groups="drop") %>%
...
recyears <- recessions %>% 
  ...
  filter(year >= 1960) %>%
  group_by(year) %>%
  summarise(year=(year - rrange + 1):(year + rrange + 1), .groups="drop") %>%
  mutate(recession=TRUE)
...
recdata %>%
  ...
  summarise(pdtrendiqr=IQR(pdtrend), .groups="drop") %>%
...
# , "tottax"
df <- vbase %>%
  filter(realnom=="nominal", name %in% c("iit", "gst", "tottax"), year >= 2000) %>%
  select(stabbr, name, year, pdtrend) %>%
  na.omit() %>%
...
df2 <- df %>%
  group_by(stabbr) %>%
  summarize(iitgst = cor(iit, gst, use="pairwise.complete.obs"),
            totiit = cor(tottax, iit, use="pairwise.complete.obs"),
            totgst = cor(tottax, gst, use="pairwise.complete.obs"),
            .groups="drop") 
df2
...
st <- "IA"
df %>%
  filter(stabbr==st, !(is.na(gst) & is.na(iit))) %>%
...
gsubtitle <- "50 states pooled, box widths indicate interquartile range of % change"
p <- pdata %>%
  # mutate(period=factor(period, levels=unique(pdata$period))) %>%
...
```{r fig_pch_taxtype_box, eval=FALSE}
