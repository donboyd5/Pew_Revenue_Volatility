...
hp_df <- function(df, vname, smooth=NULL){
  # return data frame with trend, cycle added as new variables
  vts <- ts(df[vname])
...
# construct total tax measures for each state, raw and adjusted
df <- pewtax %>%
  filter(vartype %in% c("raw", "adjusted"), taxtype=="tottax") %>%
  select(stabbr, year, vartype, value)
...
f <- function(tax, taxlabel){
  p <- pewtax %>%
        filter(taxtype==tax) %>%
        select(stabbr, vartype, year, value) %>%
        ...
        ggplot(aes(year, value, colour=vartype)) +
        geom_point() +
        geom_line() +
        scale_colour_manual(values=colors) +
...
df <- pewtax %>%
  ...
  select(stabbr, year, vartype, taxtype, pch=value) %>%
  group_by(stabbr, vartype, taxtype) %>%
  arrange(year) %>%
  mutate(value=cumprod(1 + pch) / (1 + pch[1])) %>%
  ungroup
df

df %>% filter(stabbr=="NY", taxtype=="tottax", vartype=="raw")
