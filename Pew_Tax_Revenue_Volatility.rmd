...
p <- pdata %>%
  ...
  geom_point(colour="blue", size=.75) +
    # geom_text_repel(aes(label=stabbr), size=2, colour="blue",
    #         nudge_x=.001, data=. %>% filter(tottax < .13)) +
  geom_text(aes(label=stabbr), size=2, colour="blue",
            nudge_x=.00075) +
            # position=position_dodge(width=-.05)) + #nudge_x=.001) +
  ...
  labs(x="State GDP volatility",
       title="State tax revenue and GDP volatility, 2000-2020",
