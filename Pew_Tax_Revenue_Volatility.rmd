...
# CO sevtax  2017 0
# OH cit 2014 -118 
tax1 %>% filter(stabbr=="AK", name=="iit")
tax1 %>% filter(stabbr=="AK", name=="gst")
tax1 %>% filter(stabbr=="AZ", name=="sevtax")
tax1 %>% filter(stabbr=="CO", name=="sevtax")
tax1 %>% filter(stabbr=="OH", name=="cit")
tax1 %>% filter(stabbr=="NH", name=="sevtax")
tax1 %>% filter(stabbr=="SD", name=="sevtax")
...
lastgap <- function(gap) {
  ifelse(all(gap !=TRUE, na.rm=TRUE),
         -Inf, 
         tail(which(gap==TRUE), 1)) # first gap
}


anomalies <- tax1 %>%
  group_by(stabbr, name) %>%
  mutate(missval = is.na(value),
         zeroval = value == 0,
         negval = value < 0,
         leadzero = row_number() < firstnz(value),
         trailzero = row_number() > lastnz(value),
         gap = zeroval & !(leadzero | trailzero),
         pregap = row_number() < firstgap(gap),   # before the first gap
         postgap = row_number() > lastgap(gap))   # after the last gap
summary(anomalies)
