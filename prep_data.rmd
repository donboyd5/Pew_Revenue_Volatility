...
pkgsource_dir <- r"(C:\Users\donbo\Dropbox\package_sources)" 
...
#.. get depth ----
glimpse(cigxm)
count(cigxm %>% filter(valtype=="chain"), name, vname, vdesc)

rgdpq <- cigxm %>%
  filter(name=="rgdp", valtype=="chain", freq=="Q") %>%
  select(name, vname, vdesc, date, value)

rec_depth <- recessions %>%
  filter(!is.na(rec_year)) %>%
  left_join(rgdpq %>%
              select(peak_quarter=date,
                     peak_rgdp=value), 
            by = "peak_quarter") %>%
  left_join(rgdpq %>%
              select(trough_quarter=date,
                     trough_rgdp=value), 
            by = "trough_quarter") %>%
  filter(!is.na(peak_rgdp)) %>%
  mutate(depth=trough_rgdp / peak_rgdp - 1)

rec_depthdur <- rec_duration %>%
  left_join(rec_depth %>%
              select(rec_year, peak_rgdp, trough_rgdp, depth), 
            by = "rec_year")

rec_depthdur %>%
  ggplot(aes(rec_year, duration)) +
  geom_point() +
...
#.. get cigxm shares ----
# get wide cigxm
# cigxmvars <- c("rgdp", "rpce", "rgpdi", "rgov", "rnetx")
cigxm_vars <- c("rgdp", "rpce", "rgpdi", "rgov", "rx", "rm")
# check to make sure things look ok
cigxmw <- cigxm %>%
  filter(valtype=="chain") %>%
  filter(name %in% cigxm_vars) %>%
  select(date, name, value) %>%
  pivot_wider() %>%
  mutate(cigxm=rpce + naz(rgpdi) + naz(rgov) + naz(rx) - naz(rm),
         pct=cigxm / rgdp)
summary(cigxmw)
# ok but we don't have real net exports until 1970

# we are finally ready to get shares of GDP change
cigxm_base <- cigxm %>%
  filter(valtype=="chain") %>%
  filter(name %in% cigxm_vars) %>%
  select(date, name, value) %>%
  mutate(value=ifelse(name=="rm", -value, value)) # so we can sum negative imports

rec_cigxm1 <- recessions %>%
  filter(!is.na(rec_year)) %>%
  select(rec_year, peak=peak_quarter, trough=trough_quarter) %>%
  pivot_longer(-rec_year, names_to = "pt", values_to = "date") %>%
  left_join(cigxm_base, by = "date") %>%
  filter(!is.na(name)) %>%
  select(-date) %>%
  pivot_wider(names_from = pt) %>%
  mutate(change=trough - peak) 

check <- rec_cigxm1 %>%
  group_by(rec_year) %>%
  mutate(checkpeak=sum(peak[name != "rgdp"]),
         checktrough=sum(trough[name != "rgdp"]),
         checkchange=sum(change[name != "rgdp"])) %>%
  ungroup %>%
  filter(name=="rgdp")
# a little sloppy due to residual

rec_cigxm2 <- rec_cigxm1 %>%
  filter(name!="rgdp") %>%
  group_by(rec_year) %>%
  mutate(share=-change / sum(change)) %>%
  select(rec_year, name, share) %>%
  pivot_wider(values_from = share)

vars <- c("rgdp", "rpce", "rgpdi")
labs <- c("GDP", "Consumption", "Investment")

rec_cigxm <- rec_cigxm1 %>%
  filter(name %in% vars) %>%
  mutate(namef=factor(name, levels=vars, labels=labs)) %>%
  group_by(rec_year) %>%
  mutate(change_vspeakgdp=change / peak[name=="rgdp"],
         change_vsgdpchange=change / change[name=="rgdp"]) %>%
  ungroup

rec_cigxm %>%
  filter(rec_year >= 1965) %>%
  arrange(rec_year, namef) %>%
  ggplot(aes(as.factor(rec_year), change_vspeakgdp, fill=namef)) +
  geom_col(position="dodge") +
  ggtitle("Change in real GDP, consumption, and investment as % of peak-quarter GDP",
          subtitle="Recent recessions") +
  theme_bw() +
  labs(x=NULL) +
  scale_y_continuous(name="% of peak-quarter GDP",
                     breaks=seq(-.3, .3, .02),
...
#   mutate(namef=factor(name, levels=vars, labels=labs)) %>%
#   arrange(rec_year, namef) %>%
#   ggplot(aes(as.factor(rec_year), change_vsgdpchange, fill=namef)) +
#   geom_col(position="dodge") +
#   ggtitle("Change in real GDP, consumption, and investment as % of peak-quarter GDP",
#           subtitle="Recent recessions") +
#   theme_bw() +
...
save(gdpfy, gdppi, recessions, rec_cigxm, marked_qtrs, marked_fyears,
     cigxm, agi, capgains,
